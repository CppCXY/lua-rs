...
`register_type` is a method on `LuaState` that creates a class table in the Lua global environment, exposing a Rust type's associated functions (such as constructors) to Lua.
...
state.register_type("Entity", Entity::__lua_static_methods())?;
```
