...
- 现有 direct-entry 回归测试已经覆盖主要循环家族，能够直接验证机器码入口，而不是只验证“能编译”；当前基线已经推进到 `cargo test -p luars jit:: --features jit` 173 条全绿。
- 最近一轮主线推进继续沿着同一条主线前进了四步：
   ...
   当前基线已经更新到 `cargo test -p luars jit:: --features jit` 173 条全绿；`cargo test --all-features` 也已在本工作区通过（744/744）。
