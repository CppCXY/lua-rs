...
- 换句话说，沿着“优先做通用而非 benchmark 专属”的标准，generic-for / `TFOR` 热点仍然是合理的 executor-family 试验田，但它已经不再是上一个阶段那种“完全未落地的下一刀”。在当前代码状态下，`TFORPREP -> body(arith) -> TFORCALL -> TFORLOOP` 这条单-call generic-for skeleton 已经进入可执行阶段；下一步是否继续扩它，应该由新的 trace 样本决定，而不是继续沿用旧的 `pc=57/92/127/162/273/310/346` / `pc=193/196` 快照做判断。
- 进一步往实现层收窄后，这个 `TFOR` family 的第一刀已经落成，而且 call-shaped loop body 现在也不只剩 `TForCall` 一条 lane。解释器里 `TForCall` 的热路径本质上是“把 iterator/state/control 搬到调用位，然后执行一次 `precall(..., nargs=2, nresults=C)`”；当前 native backend 已经为这条固定语义提供了专用 helper / executor。同时，最小的 `Call -> ForLoop` 形状也已经单独落成 `NativeCallForLoop`，说明当前代码基线已经从“只有 numeric loop family 可执行”推进到“窄而固定语义的 call-loop family 也可执行”。
...
- 这条最小 skeleton 现在已经落地：native backend 新增了 `NativeTForLoop` family，并接入了一个专用 `TForCall` helper。当前语义边界是：
