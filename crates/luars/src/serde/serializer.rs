...
fn to_value_internal(lua_value: &LuaValue, visited: &mut HashSet<usize>) -> Result<JsonValue, String> {
    match lua_value {
        // Simple types
        _ if lua_value.is_nil() => Ok(JsonValue::Null),
        
        _ if lua_value.is_boolean() => {
            Ok(JsonValue::Bool(lua_value.as_bool().unwrap()))
        }
        ...
        _ if lua_value.is_table() => {
            ...
            let table_ptr = lua_value.as_table_ptr()
        ...
        _ if lua_value.is_function() => {
            Err("Cannot serialize Lua function to JSON".to_string())
        }
        
        _ if lua_value.is_thread() => {
            Err("Cannot serialize Lua thread to JSON".to_string())
        }
        
        _ if lua_value.is_userdata() => {
            Err("Cannot serialize Lua userdata to JSON".to_string())
        }
        
        _ => Err(format!("Unsupported Lua type for JSON serialization: {:?}", lua_value))
    }
}
...
fn table_to_json_array(table: &crate::lua_value::LuaTable, visited: &mut HashSet<usize>) -> Result<JsonValue, String> {
...
fn table_to_json_object(table: &crate::lua_value::LuaTable, visited: &mut HashSet<usize>) -> Result<JsonValue, String> {
