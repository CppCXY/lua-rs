...
fn compile_numeric_table_shift_jmp_loop(
    ...
    let load_current = &ir.insts[2];
    let compare_guard = &ir.insts[3];
    let compare_jump = &ir.insts[4];
    let next_index = &ir.insts[5];
    let next_index_mm = &ir.insts[6];
    let reload_current = &ir.insts[7];
    let shift_write = &ir.insts[8];
    let dec_index = &ir.insts[9];
    let dec_index_mm = &ir.insts[10];
    let backedge = &ir.insts[11];

    if !matches!(bound_guard.opcode, crate::OpCode::Le)
        || bound_jump.opcode != crate::OpCode::Jmp
        || load_current.opcode != crate::OpCode::GetTable
        || !matches!(compare_guard.opcode, crate::OpCode::Lt)
        || compare_jump.opcode != crate::OpCode::Jmp
        || next_index.opcode != crate::OpCode::AddI
        || next_index_mm.opcode != crate::OpCode::MmBinI
        || reload_current.opcode != crate::OpCode::GetTable
        || shift_write.opcode != crate::OpCode::SetTable
        || dec_index.opcode != crate::OpCode::AddI
        || dec_index_mm.opcode != crate::OpCode::MmBinI
        || backedge.opcode != crate::OpCode::Jmp
    {
        return None;
    }

    let bound_guard_inst = Instruction::from_u32(bound_guard.raw_instruction);
    let compare_guard_inst = Instruction::from_u32(compare_guard.raw_instruction);
    if bound_guard_inst.get_k() || compare_guard_inst.get_k() {
        return None;
    }

    let load_current_inst = Instruction::from_u32(load_current.raw_instruction);
    let reload_current_inst = Instruction::from_u32(reload_current.raw_instruction);
    if load_current_inst.get_k() || reload_current_inst.get_k() {
        return None;
    }

    let shift_write_inst = Instruction::from_u32(shift_write.raw_instruction);
    if shift_write_inst.get_k() {
        return None;
    }

    let table_reg = load_current_inst.get_b();
    let left_bound_reg = bound_guard_inst.get_a();
    let index_reg = bound_guard_inst.get_b();
    let value_reg = compare_guard_inst.get_a();
    let temp_reg = load_current_inst.get_a();

    let next_index_inst = Instruction::from_u32(next_index.raw_instruction);
    let next_index_mm_inst = Instruction::from_u32(next_index_mm.raw_instruction);
    let dec_index_inst = Instruction::from_u32(dec_index.raw_instruction);
    let dec_index_mm_inst = Instruction::from_u32(dec_index_mm.raw_instruction);
    let bound_jump_inst = Instruction::from_u32(bound_jump.raw_instruction);
    let backedge_inst = Instruction::from_u32(backedge.raw_instruction);

    if bound_jump_inst.get_sj() <= 0 || backedge_inst.get_sj() >= 0 {
        return None;
    }

    if load_current_inst.get_c() != index_reg
        || compare_guard_inst.get_b() != temp_reg
        || reload_current_inst.get_b() != table_reg
        || reload_current_inst.get_c() != index_reg
        || reload_current_inst.get_a() != shift_write_inst.get_c()
        || shift_write_inst.get_a() != table_reg
        || shift_write_inst.get_b() != next_index_inst.get_a()
        || next_index_inst.get_b() != index_reg
        || next_index_inst.get_sc() != 1
        || next_index_mm_inst.get_a() != index_reg
        || next_index_mm_inst.get_sb() != 1
        || dec_index_inst.get_a() != index_reg
        || dec_index_inst.get_b() != index_reg
        || dec_index_inst.get_sc() != -1
        || dec_index_mm_inst.get_a() != index_reg
        || dec_index_mm_inst.get_sb() != 1
    {
        return None;
    }

    Some(CompiledTraceExecutor::NumericTableShiftJmpLoop {
        table_reg,
        index_reg,
        left_bound_reg,
        value_reg,
