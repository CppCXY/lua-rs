...
#[cfg(feature = "jit")]
use super::{finish_trace_exit, record_trace_hits_or_fallback};
...
pub(crate) unsafe fn jit_execute_linear_int_jmp_loop(
    ...
    loop {
        ...
        if let jit::LinearIntLoopGuard::HeadRegReg { .. } = guard
            && !linear_int_guard_holds(sp, base, guard)
        {
            return Some(unsafe {
                finish_trace_exit(lua_state, ci, base, target_pc, trace_hits, summary, exit_pc)
            });
        }
...
pub(crate) unsafe fn jit_execute_numeric_jmp_loop(
    ...
    loop {
        let sp = lua_state.stack_mut().as_mut_ptr();
        if !execute_numeric_steps(sp, base, constants, pre_steps) {
            record_trace_hits_or_fallback(lua_state, ci.chunk_ptr, target_pc, trace_hits, summary);
            return None;
        }
        if !tail_guard {
            ...
            if !guard_holds {
                if let Some(step) = exit_preset.as_ref()
                    && !execute_numeric_steps(sp, base, constants, std::slice::from_ref(step))
                {
                    record_trace_hits_or_fallback(lua_state, ci.chunk_ptr, target_pc, trace_hits, summary);
                    return None;
                }
        ...
        if !execute_numeric_steps(sp, base, constants, steps) {
            record_trace_hits_or_fallback(lua_state, ci.chunk_ptr, target_pc, trace_hits, summary);
            return None;
        }
...
pub(crate) unsafe fn jit_execute_numeric_ifelse_for_loop(
    ...
    loop {
        ...
        if !execute_numeric_steps(sp, base, constants, branch_steps) {
            record_trace_hits_or_fallback(lua_state, ci.chunk_ptr, target_pc, trace_hits, summary);
            return None;
        }
