    ...
    'startfunc: loop {
        ...
        loop {
            ...
            match instr.get_opcode() {
                ...
                OpCode::AddI => {
                    ...
                    unsafe {
                        ...
                        if pttisinteger(v1_ptr) {
                            ...
                            pc = pc.add(1); // Skip metamethod on success
                        }
                        // Slow path: float
                ...
                OpCode::Close => {
                    handle_close(lua_state, instr, base, frame_idx, unsafe {
                        pc.offset_from(code_base)
                    }
                        as usize)?;
                }
                OpCode::Tbc => {
                ...
                OpCode::SetField => {
                    ...
                    if let Some(table_ref) = ra.as_table_mut() {
                        ...
                        // Has metatable with new key: inline __newindex fast path
                        let meta = table_ref.meta_ptr();
                        if !meta.is_null() {
                            let mt = unsafe { &mut (*meta.as_mut_ptr()).data };
                            const TM_NEWINDEX_BIT: u8 = TmKind::NewIndex as u8;
                        ...
                        if !meta.is_null() {
                            ...
                            if !mt.no_tm(tm_idx) {
                                ...
                                if let Some(mm) = mt.impl_table.get_shortstr_fast(&event_key) {
                                    ...
                                    save_pc!();
                                    cold::call_c_mm_bin(
                                        lua_state, mm, p1, p2, result_reg, frame_idx,
                                    )?;
                                    restore_state!();
                                    continue;
                                } else {
                        ...
                        if !meta.is_null() {
                            ...
                            if !mt.no_tm(tm_idx) {
                                ...
                                let event_key =
                                    lua_state.vm_mut().const_strings.get_tm_value(tm_kind);
                                if let Some(mm) = mt.impl_table.get_shortstr_fast(&event_key) {
                                    let kb = unsafe { *chunk.constants.as_ptr().add(b) };
                ...
                OpCode::GetTabUp => {
                    ...
                    if let Some(val) = result {
                        ...
                        // Inline __index fast path for Lua function metamethod
                        let table_value = *upval.get_value_ref();
                ...
                OpCode::Len => {
                    ...
                    handle_len(lua_state, instr, &mut base, frame_idx, unsafe {
                        pc.offset_from(code_base)
                    }
                        as usize)?;
                }
                ...
                OpCode::Concat => {
                    ...
                    handle_concat(lua_state, instr, &mut base, frame_idx, unsafe {
                        pc.offset_from(code_base)
                    }
                        as usize)?;
                }
                ...
                OpCode::SetList => {
                    let mut pc_idx = unsafe { pc.offset_from(code_base) } as usize;
                    closure_vararg_ops::exec_setlist(
                        lua_state,
                        instr,
                        &chunk.code,
                        base,
                ...
                OpCode::Closure => {
                    handle_closure(lua_state, instr, base, frame_idx, chunk, unsafe {
                        pc.offset_from(code_base)
                    }
                        as usize)?;
                }
