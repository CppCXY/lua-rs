        ...
        loop {
            ...
            if lua_state.jit_state.state == crate::jit::trace::RecordState::Recording {
                ...
                match result {
                    ...
                    crate::jit::trace::RecordResult::LoopClosed => {
                        ...
                        lua_state
                            .jit_state
                            .finish_recording(head_chunk, head_pc, head_ctrs);
                    }
                    crate::jit::trace::RecordResult::Abort(reason) => {
                        ...
                        lua_state
                            .jit_state
                            .abort_recording(head_chunk, head_pc, head_ctrs, reason);
                    }
                }
            }
            ...
            match instr.get_opcode() {
                    ...
                    if pc < old_pc {
                        ...
                        if counter == crate::jit::JIT_COUNTER_BLACKLISTED {
                            ...
                            if let Some(fn_ptr) = trace_fn_ptr {
                                ...
                                let snap_id = unsafe {
                                    let f: crate::jit::trace_compiler::TraceFn =
                        ...
                        } else if lua_state.jit_state.state
                            != crate::jit::trace::RecordState::Recording
                        {
                            // Decrement counter
                            ...
                            if new_counter == 0 {
                                ...
                                lua_state
                                    .jit_state
                                    .start_recording(chunk_ptr, pc as u32, base);
                            }
                        }
                    }
                }
                            ...
                            if count > 0 {
                                ...
                                if !trap {
                                    ...
                                    if counter == crate::jit::JIT_COUNTER_BLACKLISTED {
                                        ...
                                        let chunk_ptr =
                                            lua_state.call_stack[frame_idx].chunk_ptr as usize;
                                        let trace_fn_ptr = lua_state
                                            .jit_state
                                            .get_compiled(chunk_ptr, pc as u32)
                                            .map(|ct| ct.fn_ptr);
                ...
                OpCode::ForPrep => {
                    ...
                    // Tier-1 loop JIT disabled — numeric for-loops now use the
                    // trace JIT (triggered by ForLoop backward jumps), which
                    // supports function call inlining and type specialization.
                }
                OpCode::TForPrep => {
