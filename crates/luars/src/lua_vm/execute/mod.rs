...
use crate::{
    GcTable,
    lua_value::{LUA_VFALSE, LUA_VTABLE, LuaUpvalue, LuaValue},
    lua_vm::{
        ...
        loop {
            ...
            match instr.get_opcode() {
                ...
                OpCode::GetUpval => {
                    ...
                    // Skip bounds check (compiler-generated bytecode is always valid)
                ...
                OpCode::SetUpval => {
                    ...
                    // Get value to set
                    // OPTIMIZED: Use set_value() for both open and closed upvalues
                ...
                OpCode::GetTabUp => {
                    ...
                    // OPTIMIZED HOT PATH: Minimal indirection for global var access
                    let a = instr.get_a() as usize;
                    let b = instr.get_b() as usize;
                    let c = instr.get_c() as usize;
                    ...
                    // Get table value by reference (no 16-byte copy)
                    let table_value = upval.get_value_ref();

                    // Fast path: direct hash lookup bypassing redundant type checks
                    // GetTabUp always uses short string keys from constants
                ...
                OpCode::SetTabUp => {
                    ...
                    // OPTIMIZED: Fast path uses single hash lookup for existing keys
                    let a = instr.get_a() as usize;
                    let b = instr.get_b() as usize;
                    let c = instr.get_c() as usize;
                    let k = instr.get_k();
                    ...
                    // Fast path: direct set for existing short string key (single hash lookup)
                    ...
                    if table_value.tt == LUA_VTABLE {
                        ...
                        if native.has_hash() && native.set_shortstr_unchecked(&key, value) {
                            // Successfully updated existing key - handle GC barrier
                            if value.is_collectable() {
                    ...
                    // Slow path: key doesn't exist yet or non-table
                    let table_value = upval.get_value_ref().clone();
