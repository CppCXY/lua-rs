/*----------------------------------------------------------------------
  New Execute Loop 閳?Raw-Pointer Stack Base Caching
...
use crate::{
    ...
    lua_vm::{
        ...
        execute::cold,
        execute::helper::{
            self, handle_pending_ops, lua_fmod, lua_idiv, lua_imod, lua_shiftl, lua_shiftr,
            luai_numpow, pfltvalue, pivalue, psetfltvalue, psetivalue, psetnilvalue, ptonumberns,
            pttisfloat, pttisinteger, setivalue, tointeger, tointegerns,
        },
...
/// Cached frame context 閳?lives on the Rust stack, not behind a pointer.
    ...
    'startfunc: loop {
        ...
        loop {
            ...
            match opcode {
                ...
                // All other opcodes -> outlined cold dispatch
                ...
                OpCode::Lt => {
                    ...
                    if ra.ttisinteger() && rb.ttisinteger() {
                        if (ra.ivalue() < rb.ivalue()) != k {
                            f.pc += 1;
                        }
                    } else if ra.ttisfloat() && rb.ttisfloat() {
                        if (ra.fltvalue() < rb.fltvalue()) != k {
                            f.pc += 1;
                        }
                ...
                OpCode::SetTable => {
                    ...
                    if let Some(table_ref) = ra.as_table_mut() {
                        ...
                        if rb.ttisinteger() && table_ref.impl_table.fast_seti_existing(rb.ivalue(), val) {
                            if val.is_collectable()
                                && let Some(gc_ptr) = ra.as_gc_ptr()
                            {
                                lua_state.gc_barrier_back(gc_ptr);
                ...
                OpCode::Len => {
                    ...
                    if rb.ttistable() {
                        ...
                        if meta.is_null() {
                            setivalue(unsafe { &mut *f.rp(a) }, table.len() as i64);
                            continue;
                        }
                        lua_state.set_frame_pc(f.frame_idx, f.pc as u32);
                        match noinline::try_len_meta(lua_state, meta, rb, f.frame_idx)? {
                            noinline::LenResult::RawLen => {
                                ...
                                setivalue(unsafe { &mut *f.rp(a) }, tbl.data.len() as i64);
                                continue;
                            }
                        ...
                        setivalue(unsafe { &mut *f.rp(a) }, s.len() as i64);
                        continue;
                    }
                    lua_state.set_frame_pc(f.frame_idx, f.pc as u32);
                ...
                // All other opcodes -> outlined cold dispatch
                _ => {
                    if dispatch_cold(lua_state, &mut f, chunk, instr, opcode, &mut trap)? {
                        continue 'startfunc;
                    }
                }
            }
...
/// Outlined handler for cold (infrequent) opcodes.
/// Returns `Ok(true)` when the caller should `continue 'startfunc`.
#[inline(never)]
fn dispatch_cold(
        ...
        OpCode::MmBin => {
            ...
            if v1.ttistable() {
                ...
                match noinline::try_mmbin_table_fast(
                    ...
                    noinline::MmBinResult::CallMm => return Ok(true),
