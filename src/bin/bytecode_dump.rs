        ...
        Ok(chunk) => {
            ...
            for (pc, &instr) in chunk.code.iter().enumerate() {
                ...
                let details = match opcode {
                    ...
                    OpCode::ForPrep => format!("R({}) -= R({}+2); PC += {} (-> {})", a, a, sbx, (pc as i32 + 1 + sbx)),
                    OpCode::ForLoop => format!("R({}) += R({}+2); if loop then PC += {} (-> {})", a, a, sbx, (pc as i32 + 1 + sbx)),
...
fn estimate_registers(chunk: &lua_rs::value::Chunk) -> usize {
    let mut max_reg = 0;
...
fn analyze_performance(chunk: &lua_rs::value::Chunk) {
    ...
    for &instr in &chunk.code {
        match Instruction::get_opcode(instr) {
            ...
            OpCode::Add | OpCode::Sub | OpCode::Mul | OpCode::Div | OpCode::Mod | OpCode::IDiv | OpCode::Pow => arithmetic_count += 1,
    ...
    println!("  Move instructions:       {} ({:.1}%)", move_count, move_count as f64 / chunk.code.len() as f64 * 100.0);
    println!("  LoadK instructions:      {} ({:.1}%)", loadk_count, loadk_count as f64 / chunk.code.len() as f64 * 100.0);
