        ...
        Ok(chunk) => {
            ...
            for (pc, &instr) in chunk.code.iter().enumerate() {
                ...
                let details = match opcode {
                    ...
                    OpCode::Eq => format!("if (R({}) == R({})) ~= {} then PC++", b, c, Instruction::get_k(instr) as i32),
                    OpCode::Lt => format!("if (R({}) < R({})) ~= {} then PC++", b, c, Instruction::get_k(instr) as i32),
                    OpCode::Le => format!("if (R({}) <= R({})) ~= {} then PC++", b, c, Instruction::get_k(instr) as i32),
                    OpCode::TestSet => format!("if R({}) then R({}) := R({}) else PC++", c, a, c),
