...
fn create_searchers_table(vm: &mut LuaVM) -> LuaValue {
    ...
    searchers.borrow_mut().raw_set(LuaValue::integer(1), LuaValue::cfunction(searcher_preload));
    searchers.borrow_mut().raw_set(LuaValue::integer(2), LuaValue::cfunction(searcher_lua));
    searchers.borrow_mut().raw_set(LuaValue::integer(3), LuaValue::cfunction(searcher_c));
    searchers.borrow_mut().raw_set(LuaValue::integer(4), LuaValue::cfunction(searcher_allinone));
...
fn searcher_preload(vm: &mut LuaVM) -> Result<MultiValue, String> {
    ...
    unsafe {
        ...
        let preload_val = package_rc.borrow().raw_get(&LuaValue::from_string_rc(
        ...
        let loader = preload_table.borrow().raw_get(&modname_key).unwrap_or(LuaValue::nil());
...
fn searcher_lua(vm: &mut LuaVM) -> Result<MultiValue, String> {
    ...
    let path = unsafe {
        ...
        let path_val = package_rc.borrow().raw_get(&LuaValue::from_string_rc(
    ...
    match result {
        ...
        None => {
            let err = format!("\n\tno file '{}'", 
                path.split(';').map(|template| {
                    template.replace('?', &modname.replace('.', "/"))
                }).collect::<Vec<_>>().join("'\n\tno file '")
            );
            Ok(MultiValue::single(LuaValue::from_string_rc(
                vm.create_string(err)
...
fn lua_file_loader(vm: &mut LuaVM) -> Result<MultiValue, String> {
    ...
    if !success {
        ...
        return Err(format!("error loading module '{}': {}", filepath, error_msg));
    }
...
fn searcher_c(vm: &mut LuaVM) -> Result<MultiValue, String> {
    ...
    let cpath = unsafe {
        ...
        let cpath_val = package_rc.borrow().raw_get(&LuaValue::from_string_rc(
...
fn package_searchpath(vm: &mut LuaVM) -> Result<MultiValue, String> {
    ...
    let sep_val = get_arg(vm, 2).unwrap_or(LuaValue::from_string_rc(
        vm.create_string(".".to_string())
    ));
    let rep_val = get_arg(vm, 3).unwrap_or(LuaValue::from_string_rc(
        vm.create_string("/".to_string())
    ));
