...
impl OpCode {
    ...
    pub fn get_mode(self) -> OpMode {
        use OpCode::*;
        match self {
            LoadI | LoadF | LoadK | LoadKX | Jmp | ForLoop | ForPrep | 
            TForPrep | TForCall | TForLoop | Closure => OpMode::IABx,
...
impl Instruction {
    ...
    fn set_arg(i: &mut u32, v: u32, pos: u32, size: u32) {
        *i = (*i & Self::mask0(size, pos)) 
            | ((v << pos) & Self::mask1(size, pos));
    }
    ...
    pub fn create_sj(op: OpCode, sj: i32) -> u32 {
        ((op as u32) << Self::POS_OP)
            | (((sj + Self::OFFSET_SJ) as u32) << Self::POS_SJ)
    }
