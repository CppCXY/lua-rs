...
pub fn ffi_call_wrapper(vm: &mut LuaVM) -> Result<MultiValue, String> {
...
pub fn ffi_cast(vm: &mut LuaVM) -> Result<MultiValue, String> {
...
pub fn ffi_istype(vm: &mut LuaVM) -> Result<MultiValue, String> {
...
pub fn ffi_copy(vm: &mut LuaVM) -> Result<MultiValue, String> {
    let dst_val = crate::lib_registry::get_arg(vm, 0)
        .ok_or_else(|| "ffi.copy() requires argument 1".to_string())?;
    ...
    if let (Some(dst_ptr), Some(src_ptr)) = (dst_val.as_integer(), src_val.as_integer()) {
        ...
        unsafe {
            if let Some(src_str) = src_val.as_string() {
                ...
                Err("ffi.copy() invalid arguments".to_string())
            }
        }
    }
}
...
pub fn ffi_fill(vm: &mut LuaVM) -> Result<MultiValue, String> {
    let dst_val = crate::lib_registry::get_arg(vm, 0)
