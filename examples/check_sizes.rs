use lua_rs::{LuaVM, lua_vm::LuaCallFrame, LuaFunction};
use std::mem::size_of;

fn main() {
    println!("=== Structure Sizes ===");
    println!("LuaCallFrame: {} bytes", size_of::<LuaCallFrame>());
    println!("  - Rc<LuaFunction>: {} bytes", size_of::<std::rc::Rc<LuaFunction>>());
    ...
    println!("  function: {} bytes", size_of::<std::rc::Rc<LuaFunction>>());
    ...
    let total = size_of::<usize>() * 9 + size_of::<std::rc::Rc<LuaFunction>>() + size_of::<Option<&'static str>>() * 2 + size_of::<bool>();
    println!();
    ...
    println!("Saved: {} bytes ({:.1}%)", 128 - size_of::<LuaCallFrame>(), 
